...
wine <- read.csv("/Users/zeyichen/Desktop/Courses/Y2S2/Data Analysis & AI/Projects/winequality-white.csv", sep = ";", head = TRUE)
head(wine)
dim(wine)
attach(wine)
...
pairs.panels(wine[, 1:11],
             ...
             hist.col = "steelblue",
             pch = 21, bg = c("pink", "light green", "light blue")[(iris$Species)], density = TRUE, # show density plots
...
wine1 <- wine[-c(1527,1664,3902,2782),]
set.seed(105)
training.idx1 <- sample(1: nrow(wine1), nrow(wine1)*0.8) 
train.data1 <- wine1[training.idx, ]
test.data1 <- wine1[-training.idx, ]

lmodel2 <- lm(density~fixed.acidity+volatile.acidity+citric.acid+I(residual.sugar^2)+
                chlorides+I(total.sulfur.dioxide^2)+ pH+
                sulphates+I(residual.sugar*total.sulfur.dioxide), data = train.data1)
summary(lmodel2)
